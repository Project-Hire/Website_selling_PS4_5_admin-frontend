...
const UploadFormItem = (props) => {
  const { uploadTitle, className, onRemove, uploadState, setUploadState, limit, accept, formItemProps } = props
  ...
  const beforeUpload = (file) => {
    ...
    if (!isJpgOrPng) {
      openNotification(`${file.name} không phải định dạng jpeg, jpg hoặc png!`, 'error')
    }
    const isLt2M = file.size / 1024 / 1024 < 2
    if (!isLt2M) {
      openNotification('Vui lòng tải ảnh có dung lượng nhỏ hơn 2MB!', 'error')
    }
  ...
  const uploadButton = (
    <div className="uploader">
      <div className="justify-content-center text-center">
        ...
        <div className="upload-title mt-3"> {uploadTitle || 'Add Picture'}</div>
      </div>
    </div>
  )
